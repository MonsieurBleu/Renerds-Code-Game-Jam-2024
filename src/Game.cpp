...
void Game::init(int paramSample) {
  ...
  finalProcessingStage = ShaderProgram(
      ...
      "shader/post-process/basic.vert", "", globals.standartShaderUniform2D());

  finalProcessingStage.addUniform(ShaderUniform(Bloom.getIsEnableAddr(), 10));

  camera.init(radians(70.0f), globals.windowWidth(), globals.windowHeight(),
              0.1f, 1E4f);
  ...
  depthOnlyMaterial = MeshMaterial(new ShaderProgram(
      "shader/depthOnly.frag", "shader/foward/basic.vert", ""));

  depthOnlyStencilMaterial = MeshMaterial(new ShaderProgram(
      "shader/depthOnlyStencil.frag", "shader/foward/basic.vert", ""));

  depthOnlyInstancedMaterial = MeshMaterial(new ShaderProgram(
      "shader/depthOnlyStencil.frag", "shader/foward/basicInstance.vert", ""));
  ...
  GameGlobals::PBR = MeshMaterial(new ShaderProgram(
      "shader/foward/PBR.frag", "shader/foward/basic.vert", "", uniforms3D));

  GameGlobals::PBRstencil = MeshMaterial(new ShaderProgram(
      "shader/foward/PBR.frag", "shader/foward/basic.vert", "", uniforms3D));

  GameGlobals::PBRinstanced = MeshMaterial(new ShaderProgram(
      "shader/foward/PBR.frag", "shader/foward/basicInstance.vert", "",
      globals.standartShaderUniform3D()));

  skyboxMaterial = MeshMaterial(new ShaderProgram(
      "shader/foward/Skybox.frag", "shader/foward/basic.vert", "", uniforms3D));
  ...
  FUIfont->setAtlas(
      Texture2D().loadFromFileKTX("ressources/fonts/Roboto/out.ktx"));
  defaultFontMaterial = MeshMaterial(
      new ShaderProgram("shader/2D/sprite.frag", "shader/2D/sprite.vert", "",
                        globals.standartShaderUniform2D()));

  defaultSUIMaterial = MeshMaterial(
      new ShaderProgram("shader/2D/fastui.frag", "shader/2D/fastui.vert", "",
                        globals.standartShaderUniform2D()));
...
void Game::mainloop() {
  ...
  Texture2D skyboxNightTexture =
      Texture2D().loadFromFileKTX("ressources/models/skybox/8k_starsCE.ktx");

  Texture2D skyboxReflectTexture =
      Texture2D().loadFromFile("ressources/models/skybox/reflect.png");
  ...
  for (int i = -gridSize; i < gridSize; i++)
    for (int j = -gridSize; j < gridSize; j++) {
      ...
      f->state.scaleScalar(gridScale).setPosition(
          vec3(i * gridScale * 1.80, 0, j * gridScale * 1.80));
      scene.add(f);
    }
  ...
  Texture2D leafTexture =
      Texture2D().loadFromFileKTX("ressources/models/oak/textures/leaf/CE.ktx");
  Texture2D leafTextureMat = Texture2D().loadFromFileKTX(
      "ressources/models/oak/textures/leaf/NRM.ktx");
  Texture2D trunkTexture = Texture2D().loadFromFileKTX(
      "ressources/models/oak/textures/trunc/CE.ktx");
  Texture2D trunkTextureMat = Texture2D().loadFromFileKTX(
      "ressources/models/oak/textures/trunc/NRM.ktx");
  ...
  largeTrunk->loadFromFolder("ressources/models/oak/large/trunc/", false,
                             false);
  mediumTrunk->loadFromFolder("ressources/models/oak/med/trunc/", false, false);
  smallTrunk->loadFromFolder("ressources/models/oak/small/trunc/", false,
                             false);
  ...
  generateTreesFromHeatMap(scene, "../build/ressources/treeMap.png", trunkSizes,
                           leavesSizes);
  ...
  */
  ...
  SceneDirectionalLight sun =
      newDirectionLight(DirectionLight()
  ...
  AABBCollider aabbCollider =
      AABBCollider(vec3(-32 * 50, -.15, -32 * 50), vec3(32 * 50, .1, 32 * 50));

  RigidBodyRef FloorBody = newRigidBody(
      vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0), quat(0.0, 0.0, 0.0, 1.0),
      vec3(0.0, 0.0, 0.0), &aabbCollider, PhysicsMaterial(), 0.0, false);
  ...
  RigidBodyRef playerBody = newRigidBody(
      vec3(0.0, 8.0, 0.0), vec3(0.0, 0.0, 0.0), quat(0.0, 0.0, 0.0, 1.0),
      vec3(0.0, 0.0, 0.0), &playerCollider,
      PhysicsMaterial(0.0f, 0.5f, 0.0f, 0.0f), 1.0, true);

  physicsEngine.addObject(playerBody);

  player = std::make_shared<Player>(window, playerBody, &camera, &inputs);
  ...
  lanterne->state.scaleScalar(0.01).setPosition(vec3(2, 2, 0));
  scene.add(lanterne);
